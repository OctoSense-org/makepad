...
impl Widget for WebCard {
    ...
    fn text(&self) -> String {
        if self.html.is_empty() { self.url.clone() } else { self.html.clone() }
    }
...
impl WebCardRef {
    /// Navigate to an external document without granting the inline app bridge.
    pub fn navigate(&self, cx: &mut Cx, url: &str) {
        if let Some(mut inner) = self.borrow_mut() {
            cx.stop_timer(inner.settle_timer);
            inner.html.clear();
            inner.loaded_html.clear();
            inner.url = url.to_owned();
            inner.bridge_allowed = false;
            OVERLAY_OWNER.store(inner.uid.0, Ordering::SeqCst);
            inner.redraw(cx);
        }
    }
