...
impl Widget for Dock {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Event::DragEnd = event {
            ...
            cx.draw_lists[ghost_dl_id].clear_draw_items(redraw_id, recording_gen, uniforms_gen);
            ...
            let recording_gen = cx.next_uniform_gen();
            let uniforms_gen = cx.next_uniform_gen();
            cx.draw_lists[drop_dl_id].clear_draw_items(redraw_id, recording_gen, uniforms_gen);
            self.area.redraw(cx);
        }
        ...
        match event.drag_hits(cx, self.area) {
            ...
            DragHit::Drop(f) => {
                ...
                let recording_gen = cx.next_uniform_gen();
                let uniforms_gen = cx.next_uniform_gen();
                cx.draw_lists[self.ghost_tab_draw_list.draw_list_id()].clear_draw_items(redraw_id, recording_gen, uniforms_gen);
                let recording_gen = cx.next_uniform_gen();
                let uniforms_gen = cx.next_uniform_gen();
