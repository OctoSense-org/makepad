...
pub trait WidgetNode: ScriptApply {
    ...
    /// The widget's material layers as (field name, live area): every
    /// `#[live]` draw-shader field, generated by the Widget derive. A design
    /// tool mirrors a layer's draw call through its area.
...
impl WidgetRef {
    ...
    */
    ...
    pub fn layer_areas(&self) -> Vec<(&'static str, Area)> {
        match self.0.try_borrow() {
            Ok(inner) => inner.as_ref().map(|w| w.widget.layer_areas()).unwrap_or_default(),
            Err(_) => Vec::new(),
        }
    }
