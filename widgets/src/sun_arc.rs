    ...
    mod.widgets.SunArc = View{
        ...
        draw_bg +: {
            ...
            pixel: fn() {
                ...
                let pad = w * 0.07
                // `half` is a Metal scalar type, so it cannot name a local.
                let half_span = (w - pad * 2.0) * 0.5
                let rise = h * 0.52
                let hy = h * 0.74
                let rr = (half_span * half_span + rise * rise) / (2.0 * rise)
                let ccx = w * 0.5
                let ccy = hy - rise + rr
                ...
                // sin(theta) = half_span / rr.
                let t = clamp(self.progress, 0.0, 1.0)
                let th = asin(clamp(half_span / rr, 0.0, 1.0))
