...
fn get_sdk_home(deveco_home: &Path, _host_os: &HostOs) -> Result<PathBuf, String> {
    ...
    for i in 1..=5 {
        let sdk = deveco_home
            .join("sdk")
            .join(format!("HarmonyOS-NEXT-DB{i}"))
            .join("openharmony");
        if sdk.is_dir() {
            return Ok(sdk);
        }
    ...
    return Err(format!(
        "failed to get sdk home, deveco_home={}",
        deveco_home.to_str().unwrap()
    ));
}
...
fn get_deveco_sdk_home(deveco_home: &Path, _host_os: &HostOs) -> Result<PathBuf, String> {
    ...
    return Err(format!(
        "failed to get deveco sdk home, deveco_home={}",
        deveco_home.to_str().unwrap()
    ));
}
...
fn get_node_home(deveco_home: &Path, host_os: &HostOs) -> Result<PathBuf, String> {
    match host_os {
        HostOs::Linux => {
            ...
            return Err(format!(
                "failed to get node home, deveco_home={}",
                deveco_home.to_str().unwrap()
            ));
        }
...
fn get_node_path(deveco_home: &Path, host_os: &HostOs) -> Result<PathBuf, String> {
    match host_os {
        HostOs::Linux => {
            ...
            return Err(format!(
                "failed to get node path, deveco_home={}",
                deveco_home.to_str().unwrap()
            ));
        }
        HostOs::WindowsX64 => {
...
fn get_hvigor_path(deveco_home: &Path, host_os: &HostOs) -> Result<PathBuf, String> {
    match host_os {
        HostOs::Linux => {
            ...
            return Err(format!(
                "failed to get hvigor path, deveco_home={}",
                deveco_home.to_str().unwrap()
            ));
        }
        HostOs::WindowsX64 => {
...
fn get_hdc_path(deveco_home: &Path, host_os: &HostOs) -> Result<PathBuf, String> {
    match host_os {
        HostOs::Linux | HostOs::MacOS => {
            ...
            for i in 1..-5 {
                let hdc_path = deveco_home.join(format!(
                    "sdk/HarmonyOS-NEXT-DB{i}/openharmony/toolchains/hdc"
                ));
                if hdc_path.is_file() {
                    return Ok(hdc_path);
                }
        ...
        HostOs::WindowsX64 => {
            let hdc_path =
                deveco_home.join(format!("sdk\\default\\openharmony\\toolchains\\hdc.exe"));
            if hdc_path.is_file() {
                return Ok(hdc_path);
            }
            for i in 1..=5 {
                let hdc_path = deveco_home.join(format!(
                    "sdk\\HarmonyOS-NEXT-DB{i}\\openharmony\\toolchains\\hdc.exe"
                ));
                if hdc_path.is_file() {
                    return Ok(hdc_path);
                }
    ...
    return Err(format!(
        "failed to get hdc path, deveco_home={}",
        deveco_home.to_str().unwrap()
    ));
}
...
pub fn rust_build(
    ...
    for target in targets {
        ...
        let full_clang_path =
            native_llvm_bin.join(bin_path(&format!("{target_triple}-clang"), "cmd"));
        let full_clangcl_path =
            native_llvm_bin.join(bin_path(&format!("{target_triple}-clang-cl"), "cmd"));
        let full_clangpp_path =
            native_llvm_bin.join(bin_path(&format!("{target_triple}-clang++"), "cmd"));
        ...
        let cc_path = if matches!(host_os, HostOs::WindowsX64) {
            if !full_clang_path.is_file()
                || !full_clangcl_path.is_file()
                || !full_clangpp_path.is_file()
            {
...
fn create_deveco_project(args: &[String], targets: &[OpenHarmonyTarget]) -> Result<(), String> {
    ...
    let project_output_path = cwd
        .join("target")
        .join("makepad-open-harmony")
        .join(&underscore_build_crate);
    let resources_rawfile = project_output_path
        .join("entry")
        .join("src")
        .join("main")
        .join("resources")
    ...
    let project_template_path = {
        ...
        let cwd = std::env::current_dir().unwrap();
        let tools_openharmony_dir = cwd
            .join(cargo_makepad_installation_path)
            .parent()
    ...
    let lable_path = project_output_path
        .join("entry")
        .join("src")
        .join("main")
        .join("resources")
    ...
    let label_us_path = project_output_path
        .join("entry")
        .join("src")
        .join("main")
        .join("resources")
    ...
    let label_zh_path = project_output_path
        .join("entry")
        .join("src")
        .join("main")
        .join("resources")
...
fn build_hap(
    ...
    shell_env(
        &[
            (
                &format!("DEVECO_SDK_HOME"),
                deveco_sdk_home.to_str().unwrap(),
            ),
...
pub fn run(
    ...
    let prj_path = cwd
        ...
        .join("makepad-open-harmony")
        .join(&underscore_build_crate);
    let hap_path = prj_path
        .join("entry")
        .join("build")
        .join("default")
        .join("outputs")
        .join("default")
        .join("makepad-default-signed.hap");
    ...
    hdc_cmd(
        &hdc,
        &prj_path,
        &["shell", "aa", "force-stop", bundle.as_str()],
        &hdc_remote,
    )?;
    ...
    let _ = hdc_cmd(
        &hdc,
        &prj_path,
        &["shell", "rm", "-rf", bundle_dir.as_str()],
        &hdc_remote,
    );
    hdc_cmd(
        &hdc,
        &prj_path,
        &["shell", "mkdir", bundle_dir.as_str()],
        &hdc_remote,
    )?;
    hdc_cmd(
        &hdc,
    ...
    hdc_cmd(
        &hdc,
        &prj_path,
        &["shell", "bm", "install", "-p", bundle_dir.as_str()],
        &hdc_remote,
    )?;
    hdc_cmd(
        &hdc,
