...
impl CxVulkan {
    ...
    pub(super) fn new_offscreen_on(uuid: Option<[u8; 16]>) -> Result<Self, String> {
        ...
        // This fork has no runtime OpenGL fallback on desktop Linux (upstream's
        // `gpu_preference`), so a software rasterizer stays usable, ranked last.
        let allow_cpu = true;
    ...
    pub fn new_wayland(display: *mut c_void) -> Result<Self, String> {
        ...
        // No runtime OpenGL fallback here (see `new_offscreen_on`).
        let allow_cpu = true;
