    ...
    ($app:ident, $configure:expr) => {{
        ...
        Box::new(move |cx: &mut Cx, event: &Event| {
            ...
            if let Event::LiveEdit = event {
                // `ScriptReapply` for a stylesheet reapply, otherwise `Reload`
                // when the DSL changed on disk or `Rebake` when `script_mod`
                // only re-ran to pick up new heap primitives.
                let style_reload = std::mem::take(&mut cx.pending_style_reload);
                let live_edit_apply = if style_reload { $crate::Apply::ScriptReapply } else { cx.live_edit_apply() };
