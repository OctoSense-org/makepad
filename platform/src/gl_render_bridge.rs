...
pub struct GlRenderBridge {
    ...
    #[cfg(all(target_os = "ios", not(gpusim)))]
...
// Cx methods: Linux (desktop; OpenHarmony keeps its EGL state on its display, below)
...
// Cx methods: OpenHarmony — the EGL context lives on the ArkUI XComponent display.
...
impl Cx {
    ...
    /// Restore makepad's GL state after external GL rendering (shared EGL context, as on Android).
    pub fn restore_gl_context(&mut self) {
        let display = self
            .os
            .display
